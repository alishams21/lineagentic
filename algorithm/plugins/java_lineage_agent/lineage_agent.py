...
from ...plugins.java_lineage_agent.java_instructions import (syntax_analysis_instructions,
...
class JavaLineageAgent:
    ...
    async def create_agent(self, java_mcp_servers, instructions) -> Agent:
        agent = Agent(
    ...
    async def run_agent(self, java_mcp_servers, query: str):
        # Step 1: Run structure parsing agent first
        syntax_analysis_agent = await self.create_agent(java_mcp_servers, syntax_analysis_instructions(self.agent_name))
        ...
        field_derivation_agent = await self.create_agent(java_mcp_servers, field_derivation_instructions(self.agent_name))
        operation_tracing_agent = await self.create_agent(java_mcp_servers, operation_tracing_instructions(self.agent_name))
        ...
        event_composer_agent = await self.create_agent(java_mcp_servers, event_composer_instructions(self.agent_name))
        
        # Combine all outputs for the aggregation agent
        combined_output = f"""
        Parsed Java Blocks Output:
        {syntax_analysis_output}
    ...
    async def run_with_mcp_servers(self, query: str):
        async with AsyncExitStack() as stack:
            ...
            return await self.run_agent(java_mcp_servers, query=query)
...
def create_java_lineage_agent(agent_name: str, query: str, model_name: str = "gpt-4o-mini") -> JavaLineageAgent:
    """Factory function to create a JavaLineageAgent instance"""
    return JavaLineageAgent(agent_name=agent_name, query=query, model_name=model_name)
