...
class AirflowLineageAPIClient:
    ...
    def analyze_query(self, source_code: str, model_name: str = "gpt-4o-mini", 
                     agent_name: str = "airflow") -> Dict[str, Any]:
        """
        Analyze a single Airflow query using the airflow_lineage_agent plugin
        
        Args:
            source_code: Airflow query to analyze
...
def main():
    ...
    print("Analyzing Airflow query...")
    lineage_result = client.analyze_query(
        source_code=sample_query,
        agent_name="airflow-lineage-agent"
    )
    print(f"Airflow lineage agent result: {json.dumps(lineage_result, indent=2)}")
    print()
