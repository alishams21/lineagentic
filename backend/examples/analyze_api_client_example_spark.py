...
class SparkLineageAPIClient:
    ...
    def analyze_query(self, source_code: str, model_name: str = "gpt-4o-mini", 
                     agent_name: str = "spark") -> Dict[str, Any]:
        """
        Analyze a single Spark query using the spark_lineage_agent plugin
        
        Args:
            source_code: Spark query to analyze
...
def main():
    ...
    print("Analyzing Spark query...")
    lineage_result = client.analyze_query(
        source_code=sample_query,
        agent_name="spark-lineage-agent"
    )
    print(f"Spark lineage agent result: {json.dumps(lineage_result, indent=2)}")
    print()
