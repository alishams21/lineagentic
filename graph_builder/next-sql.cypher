...
// === Create Run ===
MERGE (r:Run {runId: "sql-run-00123"})
...
// === Parent Run and Job ===
MERGE (parentRun:Run {runId: "etl-orchestrator-run-789"})
MERGE (parentJob:Job {name: "region_sales_aggregation", namespace: "sql-jobs"})
...
// === Owner 1 ===
MERGE (owner_0_0:Owner {name: "data.3@company.com", type: "group"})
...
// === Output Dataset 1 ===
MERGE (ds_out_0:Dataset {namespace: "data-lake", name: "analytics.sales_by_region"})
...
MERGE (transform_0_1_0:Transformation {
  type: "aggregation",
  subtype: "sum",
  description: "Summed total_sales per region",
  masking: false
})
